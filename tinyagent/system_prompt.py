...
RESPONSE_TYPE_DEFINITION = """Populate the response field to give simple response or when you have the final response
...
class SystemPromptBuilder:
    def __init__(self, system_prompt: str | None = None, response_type: BaseModel | None = None):
        ...
        if response_type:
            self.system_prompt['content'] += '\n\n' + jinja2.Template(RESPONSE_TYPE_DEFINITION).render(
                response_type=response_type.model_json_schema()
            )
